 ...
 * IP地址服务
 */
public class IpAddressSeeker {
    private static Logger log = LoggerManager.getLogger(IpAddressSeeker.class);

    /**
     * 纯真IP数据库名
     */
    private final String ipFilePath ;
    /**
     * 纯真IP数据库保存的文件夹
     */
    ...
    public IpAddressSeeker(String ipFilePath) {
    ...
    public void init() {
        try {
            ...
            try {
                ...
                String filename = new File(ipFilePath).getName().toLowerCase();
    ...
    public String getCountry(byte[] ip) {
        ...
        // 保存ip，转换ip字节数组为字符串形式
        String ipStr = ChunZhenUtil.getIpStringFromBytes(ip);
    ...
    public String getCountry(String ip) {
        return getCountry(ChunZhenUtil.getIpByteArrayFromString(ip));
    }
    ...
    public String getArea(final String ip) {
        return getArea(ChunZhenUtil.getIpByteArrayFromString(ip));
    }
    ...
    private String readString(long offset) {
        try {
            ...
            for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile.readByte())
                ;
            if (i != 0) {
                return ChunZhenUtil.getString(buf, 0, i, "GBK");
            }
        }
        catch (IOException e) {
            log.error(e.getMessage(), e);
    ...
    public static void main(String[] args) {
        IpAddressSeeker ip = new IpAddressSeeker("/developer/Codes/IdeaCodes/spring-boot-starter-jtweb2/Doc/chunzhen/qqwry.dat");
//        ip.init();
        String address = "112.225.35.70";
        System.out.println("IP地址[" + address + "]获取到的区域信息:" + ip.getIPLocation(
                address).getCountry() + ", 获取到的城市:" + ip.getIPLocation(
                address).getCity() + ", 运营商:" + ip.getIPLocation(address).getArea()+ip.getIPLocation(address).getCopy());
    }
